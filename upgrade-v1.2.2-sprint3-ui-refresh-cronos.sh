...
EXPECTED_VERSION="1.2.2-sprint3-ui-refresh"
...
grep -q "refreshScheduled" "$SOURCE_DIR/app/static/app-v0.9.7.js" || { echo "Correctif UI absent." >&2; exit 1; }

cp -a "$TARGET_DIR" "$BACKUP"
echo "Sauvegarde : $BACKUP"

# Mise à niveau frontend/control-plane uniquement. Ne jamais remplacer le compose,
# le .env, la base, generated/, les runtimes ou l'agent déjà validé.
...
install -m 0644 "$SOURCE_DIR/README-V1.2.2-SPRINT3-UI-REFRESH.md" "$TARGET_DIR/README-V1.2.2-SPRINT3-UI-REFRESH.md"
...
grep -q "$EXPECTED_VERSION" <<<"$HEALTH" || { echo "Version HTTP inattendue : $HEALTH" >&2; exit 1; }
