...
2. Interface **Références → Créer une référence/nouvelle version** : sélectionner explicitement le nœud et le conteneur approuvés. Vérifier que le preflight affiche données estimées, espace temporaire requis/disponible/manquant et refuse le lancement si la réserve est insuffisante.
3. Pendant l’exécution, exiger que le workflow global reste running après le SUCCESS du sous-job discovery. Vérifier `offered`, ACK, puis `claimed`; lease et activité doivent être absentes avant ACK. Pendant plusieurs intervalles de 180 s, relever périodiquement `worker_activity_at` et `lease_expires_at` : ils doivent avancer même si la progression reste à 25 %. Observer aussi la croissance de `reference.tar.gz`; aucune étape SUCCESS ne doit afficher « Non démarrée » et une durée absente doit rester « — ».
4. Contrôler que Plex garde le même StartedAt et `RestartCount=0`, puis vérifier `/identity`, journaux, validation SQLite, Metadata/Media, tailles et checksum. Vérifier le tar et son SHA sur le chemin approuvé, sans extraction dans le `/config` source. Si `schema-readable-tokenizer-unavailable` apparaît, valider les DB avec le moteur Plex du même build sur une copie de test.
...
10. Sur ce seul environnement de test, lancer puis annuler une capture : exiger `cancelled`, commande libérée, répertoire temporaire supprimé et ancienne version intacte. Relancer, puis arrêter réellement l’agent après ACK : après expiration de la lease, exiger `cancel_requested_at`, build failed, upload tardif HTTP 409, aucune publication partielle et aucun double worker au redémarrage. Vérifier que le worker revenu observe l’annulation et nettoie son staging. Vérifier aussi un espace devenu insuffisant entre discovery et capture. Enfin simuler un refus de claim. Conserver les preuves expurgées et les durées.
...
- **Capture** : la capture live ne doit jamais arrêter Plex. Si StartedAt ou RestartCount change, stopper la validation et conserver les diagnostics ; ne pas masquer cette régression par un redémarrage manuel.
