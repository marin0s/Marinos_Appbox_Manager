...
3. Pendant l’exécution, exiger que le workflow global reste running après le SUCCESS du sous-job discovery. Observer plusieurs hausses de progression pendant la croissance de `reference.tar.gz`; aucune étape SUCCESS ne doit afficher « Non démarrée » et une durée absente doit rester « — ».
...
10. Sur ce seul environnement de test, lancer puis annuler une capture : exiger `cancelled`, commande libérée, répertoire temporaire supprimé et ancienne version intacte. Relancer, puis arrêter l’agent pendant la capture : après 180 s par défaut, exiger lease expirée, build failed, aucune publication partielle et aucun double worker au redémarrage. Vérifier aussi un espace devenu insuffisant entre discovery et capture. Enfin simuler un refus de claim. Conserver les preuves expurgées et les durées.
