...
BACKUP_DIR="/opt/appbox-manager-backups/v0.9.7-$(date +%F-%H%M%S)"
...
python3 -c 'import json; p=json.load(open("/tmp/appbox-health.json")); assert p["version"]=="0.9.7"; assert p.get("remote_deployment_executor") is True; print(json.dumps(p,indent=2,ensure_ascii=False))'
...
echo "Upgrade V0.9.7 terminé. Rechargez l’interface avec Ctrl+F5."
