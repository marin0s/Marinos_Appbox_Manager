...
BACKUP="${TARGET}.backup-v1.5.2-${STAMP}"
...
cd "$TARGET"
echo "Construction de l'image applicative 1.5.2..."
...
echo "Démarrage de la version 1.5.2..."
docker compose up -d --remove-orphans

healthy=0
for _ in $(seq 1 60); do
  if curl -fsS "$HEALTH_URL" 2>/dev/null | grep -q '"version":"1.5.2"'; then
    healthy=1
    break
  fi
  sleep 2
...
if [ "$healthy" -ne 1 ]; then
  echo "Healthcheck 1.5.2 échoué." >&2
...
echo "AppBox Manager 1.5.2 Reference Images UX déployé avec succès."
