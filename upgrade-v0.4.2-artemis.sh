...
STAMP="$(date +%F-%H%M%S)"
BACKUP="/opt/appbox-manager-poc-backup-v0.4.2-${STAMP}"
...
for i in $(seq 1 60); do
  curl -fsS http://127.0.0.1:8090/health >/tmp/appbox-health-v042.json 2>/dev/null && break
  sleep 1
done

echo "[7/7] Contrôles"
python3 -m json.tool /tmp/appbox-health-v042.json
curl -fsS http://127.0.0.1:8090/ >/dev/null
curl -fsS http://127.0.0.1:8090/appboxes >/dev/null
curl -fsS http://127.0.0.1:8090/nodes/artemis >/dev/null
docker logs appbox-manager-artemis --tail 30
echo
echo "V0.4.2 installée. Backup : $BACKUP"
