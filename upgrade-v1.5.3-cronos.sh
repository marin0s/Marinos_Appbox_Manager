...
BACKUP="${TARGET}.backup-v1.5.3-${STAMP}"
...
mkdir -p \
  ...
  "$REFERENCE_DIR/deployment-cache"
chmod 750 "$REFERENCE_DIR" "$REFERENCE_DIR/plex" "$REFERENCE_DIR/jellyfin" "$REFERENCE_DIR/deployment-cache"

cd "$TARGET"
echo "Construction de l'image applicative 1.5.3..."
...
echo "Démarrage de la version 1.5.3..."
docker compose up -d --remove-orphans

healthy=0
for _ in $(seq 1 60); do
  if curl -fsS "$HEALTH_URL" 2>/dev/null | grep -q '"version":"1.5.3"'; then
    healthy=1
    break
  fi
  sleep 2
...
if [ "$healthy" -ne 1 ]; then
  echo "Healthcheck 1.5.3 échoué." >&2
...
if [ -f "$DB" ]; then
  ...
  curl -fsS "$HEALTH_URL" | grep -q '"reference_discovery":true' || fail "Capacité reference_discovery absente du healthcheck"
  curl -fsS "$HEALTH_URL" | grep -q '"reference_deployment":true' || fail "Capacité reference_deployment absente du healthcheck"
fi

echo "AppBox Manager 1.5.3 Reference Deployment déployé avec succès."
