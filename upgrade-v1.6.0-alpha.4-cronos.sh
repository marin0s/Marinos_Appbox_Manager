...
BACKUP="${TARGET}.backup-v1.6.0-alpha.4-${STAMP}"
...
cp -a "$TARGET" "$BACKUP"
echo "$BACKUP" > /root/appbox-manager-v1.6.0-alpha.4-last-backup
...
for _ in $(seq 1 60); do
 curl -fsS "$HEALTH_URL" 2>/dev/null | grep -q '"version":"1.6.0-alpha.4"' && healthy=1 && break
 sleep 2
done
[ "${healthy:-0}" -eq 1 ] || fail "Healthcheck V1.6 échoué. Rollback: $SRC_DIR/rollback-v1.6.0-alpha.4-cronos.sh"
"$SRC_DIR/verify-v1.6.0-alpha.4-cronos.sh"
echo "V1.6.0-alpha.4 installée. Backup : $BACKUP"
